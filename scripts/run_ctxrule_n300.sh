...
SEL=$OUT/sel_e1b_n300.json
REC=$OUT/ptrue_ctxrule_e1b_n300.jsonl
...
while :; do
  ...
  CUDA_VISIBLE_DEVICES=$GPU setsid nohup $V serve "$SNAP" --served-model-name qwen \
    ...
    --port $PORT >> /tmp/ctxrule_n300_serve.log 2>&1 &
  SRV=$!
  ready=0
  for i in $(seq 1 180); do
    ...
    grep -qiE "Engine core initialization failed|out of memory|ValueError|Error in inspecting model" \
      /tmp/ctxrule_n300_serve.log 2>/dev/null && break
  ...
  grep -m2 -E "ValueError|out of memory|Engine core initialization failed" /tmp/ctxrule_n300_serve.log | cut -c1-200
  cp /tmp/ctxrule_n300_serve.log /tmp/ctxrule_n300_serve.attempt$attempt.log 2>/dev/null
  FREED=0; free_gpu; FREED=0        # tear down the half-started server, then try again
  sleep 60
done

# ---- 4. sweep: 300 steps x 11 conditions = 3300 single-token calls, 11 in flight
rm -f $REC
$PY -u src/ptrue_context_rule_probe.py run \
  --selection $SEL --out $REC \
  --model qwen --tokenizer "$SNAP" --base-url http://localhost:$PORT/v1 --concurrency 11 \
  > $OUT/run_sweep_n300.log 2>&1
rc=$?
echo "[ctxrule] sweep rc=$rc lines=$(wc -l < $REC 2>/dev/null || echo 0)"
tail -3 $OUT/run_sweep_n300.log
...
$PY -u src/ptrue_context_rule_probe.py report \
  --records $REC --csv $OUT/ptrue_ctxrule_e1b_n300_matrix.csv --out $OUT/ptrue_ctxrule_e1b_n300.md
echo "CTXRULE_DONE $(date)"
echo "--- separation table ---"
sed -n '/## Separation per condition/,/^$/p;/^| \*\*C/p' $OUT/ptrue_ctxrule_e1b_n300.md | head -20
