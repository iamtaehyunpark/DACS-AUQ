...
## E1. The evidence law → proves Argument 1

Score identical steps under cumulative evidence: {action only} → {+history}
→ {+agent reasoning} → {+realized environment response}; controls:
permuted-history, wrong-episode-history (proves content, not length/format).
Two assessor models × all agents × all environments.
**Pass**: same ordering in ≥90% of arms — action ≈ chance, realized response
= largest gain, reasoning ≈ negligible; controls flat.
**Picture**: one rising line per arm, all the same shape.
...
- **D7. Master probe tensor** — (assessor × target × environment × step ×
  evidence-condition × recipe [× perturbation]) → score. E1/E3/E4/E6b are
  named slices; E5 = D7∩D4 analysis; E6a = policy simulation over D7+D2
  (zero new scoring for both). Held-out-agent = exclusion rule, not extra
  data. Volume: ~1.5–3M single-token probe calls (GPU-days, not months).
