...
SEL=$OUT/sel_horizon.json
REC=$OUT/ptrue_horizon_e1b.jsonl
...
while :; do
  ...
  CUDA_VISIBLE_DEVICES=$GPU setsid nohup $V serve "$SNAP" --served-model-name qwen \
    ...
    --port $PORT >> /tmp/horizon_serve.log 2>&1 &
  SRV=$!
  ready=0
  for i in $(seq 1 180); do
    ...
    grep -qiE "Engine core initialization failed|out of memory|ValueError|Error in inspecting model" \
      /tmp/horizon_serve.log 2>/dev/null && break
  ...
  grep -m2 -E "ValueError|out of memory|Engine core initialization failed" /tmp/horizon_serve.log | cut -c1-200
  cp /tmp/horizon_serve.log /tmp/horizon_serve.attempt$attempt.log 2>/dev/null
...
$PY -u src/ptrue_horizon_probe.py run \
  ...
  --model qwen --tokenizer "$SNAP" --base-url http://localhost:$PORT/v1 --concurrency 7 \
  > $OUT/run_horizon.log 2>&1
rc=$?
echo "[ctxrule] sweep rc=$rc lines=$(wc -l < $REC 2>/dev/null || echo 0)"
tail -3 $OUT/run_horizon.log
...
$PY -u src/ptrue_horizon_probe.py report \
  --records $REC --csv $OUT/ptrue_horizon_e1b_curves.csv --out $OUT/ptrue_horizon_e1b.md
echo "CTXRULE_DONE $(date)"
echo "--- separation table ---"
sed -n '/## Trajectory outcome/,/^$/p;/^| /p' $OUT/ptrue_horizon_e1b.md | head -40
