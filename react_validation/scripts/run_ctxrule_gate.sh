...
SEL=$OUT/sel_e1b.json
REC=$OUT/ptrue_ctxrule_e1b.jsonl
...
$PY -u src/ptrue_context_rule_probe.py select \
  --uq $CORPUS_UQ --judge $CORPUS_JUDGE --out $SEL 2>&1 | grep -v '^skip '
[ -s "$SEL" ] || { echo "SELECT_FAILED"; exit 1; }
...
while :; do
  ...
  CUDA_VISIBLE_DEVICES=$GPU setsid nohup $V serve "$SNAP" --served-model-name qwen \
    ...
    --port $PORT >> /tmp/ctxrule_serve.log 2>&1 &
  SRV=$!
  ready=0
  for i in $(seq 1 180); do
    ...
    grep -qiE "Engine core initialization failed|out of memory|ValueError|Error in inspecting model" \
      /tmp/ctxrule_serve.log 2>/dev/null && break
  ...
  grep -m2 -E "ValueError|out of memory|Engine core initialization failed" /tmp/ctxrule_serve.log | cut -c1-200
  cp /tmp/ctxrule_serve.log /tmp/ctxrule_serve.attempt$attempt.log 2>/dev/null
  FREED=0; free_gpu; FREED=0        # tear down the half-started server, then try again
  sleep 60
done

# ---- 4. sweep: 20 steps x 11 conditions, one token each
rm -f $REC
$PY -u src/ptrue_context_rule_probe.py run \
  --selection $SEL --out $REC \
  --model qwen --tokenizer "$SNAP" --base-url http://localhost:$PORT/v1 \
  > $OUT/run_sweep.log 2>&1
rc=$?
echo "[ctxrule] sweep rc=$rc lines=$(wc -l < $REC 2>/dev/null || echo 0)"
tail -3 $OUT/run_sweep.log
...
# ---- 5. report (no GPU)
$PY -u src/ptrue_context_rule_probe.py report \
  --records $REC --csv $OUT/ptrue_ctxrule_e1b_matrix.csv --out $OUT/ptrue_ctxrule_e1b.md
echo "CTXRULE_DONE $(date)"
echo "--- separation table ---"
sed -n '/## Separation per condition/,/^$/p;/^| \*\*C/p' $OUT/ptrue_ctxrule_e1b.md | head -20
