#!/usr/bin/env bash
# 10-episode PILOT — first smoke test of the HotpotQA structure on the server.
# Same pure-Python loop as run_hotpot_validation.sh, capped at 10 episodes via REACT_N_EPISODES.
# Requires: vLLM server up (serve.sh), openai, gym, beautifulsoup4, numpy, requests,
# AND outbound internet (the env hits live en.wikipedia.org on every search[]).
set -euo pipefail
cd "$(dirname "$0")/../src"

curl -s http://localhost:8000/v1/models || { echo "no vLLM server at :8000 — run serve.sh first"; exit 1; }

# REACT_NO_STOP=1 -> our modified react (unrestricted generation; harness parses the Action label).
REACT_NO_STOP=1 REACT_N_EPISODES=10 python3 react_hotpotqa.py 2>&1 | tee run_hotpot_pilot.log
echo "done -> $(pwd)/run_hotpot_pilot.log"
